...
def text_to_ids(source_text, target_text, source_vocab_to_int, target_vocab_to_int):
    ...
    for i in range(len(source_sentences)):
        texts = source_sentences[i].split()
        index_list = list()
        for j in range(len(texts)):
            index_list.append(source_vocab_to_int[texts[j]])
        source_index.append(index_list)
        
    target_index = list()
    target_sentences = target_text.split('\n')
    for i in range(len(target_sentences)):
        texts = target_sentences[i].split()
        index_list = list()
        for j in range(len(texts)):
            index_list.append(target_vocab_to_int[texts[j]])
        index_list.append(target_vocab_to_int['<EOS>'])
        target_index.append(index_list)
...
def model_inputs():
    ...
    target_tensor = tf.placeholder(dtype=tf.int32, shape=[None,None])
    learning_rate_tensor = tf.placeholder(dtype=tf.float32, shape=None)
    keep_prob_tensor = tf.placeholder(dtype=tf.float32, shape=None, name='keep_prob')
    return input_tensor, target_tensor, learning_rate_tensor, keep_prob_tensor
...
def process_decoding_input(target_data, target_vocab_to_int, batch_size):
    ...
    """
    # TODO: Implement Function
    end = tf.strided_slice(target_data,[0,0],[batch_size,-1],strides=[1,1])
    return tf.concat([tf.fill(dims=[batch_size,1],value = target_vocab_to_int['<GO>']),end],1)
...
def decoding_layer(dec_embed_input, dec_embeddings, encoder_state, vocab_size, sequence_length, rnn_size,
    ...
    with tf.variable_scope("share_variable_scope") as decoding_scope:
        output_fn = lambda output: tf.contrib.layers.fully_connected(output, vocab_size, scope=decoding_scope,activation_fn=None)
        logits_train = decoding_layer_train(encoder_state,cell,dec_embed_input,sequence_length,decoding_scope,output_fn,keep_prob)
        
    with tf.variable_scope("share_variable_scope",reuse=True) as decoding_scope:
        start_of_sequence_id = target_vocab_to_int['<GO>']
        end_of_sequence_id = target_vocab_to_int['<EOS>']
        logits_infer = decoding_layer_infer(encoder_state,cell,dec_embeddings,start_of_sequence_id,end_of_sequence_id,
...
def seq2seq_model(input_data, target_data, keep_prob, batch_size, sequence_length, source_vocab_size, target_vocab_size,
    ...
    state = encoding_layer(enc_embed_input,rnn_size,num_layers,keep_prob)
    ...
    return decoding_layer(target_embed, dec_embeddings, state,  target_vocab_size, sequence_length, rnn_size, 
                          num_layers, target_vocab_to_int, keep_prob)
...
def sentence_to_seq(sentence, vocab_to_int):
    ...
    for i in range(len(texts)):
        if texts[i] in vocab_to_int.keys():
            ids.append(vocab_to_int[texts[i]])
        else:
