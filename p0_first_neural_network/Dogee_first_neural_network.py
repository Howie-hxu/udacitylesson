...
# #### 请将你的答案填写在下方
# 模型对数据的预测效果并不是太好。  
# 根据之前代码的曲线趋势，以及训练结束后的输出结果：Training loss: 0.168 ... Validation loss: 0.316  
# 我觉得问题是出在拟合不够，根据上面的建议，我决定增加训练次数，增加隐藏神经元数量（并尝试减少学习效率）。  
# 不过效果不是很明显，所以还不清楚为何会出现问题，需要再看看  
# update 2017/08/28：
# 经过reviewer指点，修改后训练的输出结果比较好，但是在最后的预测上，误差还是蛮明显的。  
# 而最终的训练结果中，Validation loss: 0.188，有20%左右的错误率，所以整个模型还是有优化空间的。
# 
# 
